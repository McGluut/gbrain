...
describe('skillify-check ↔ gbrain check-resolvable wiring', () => {
  ...
  test('happy path: synthetic gbrain on PATH returns ok=true, gate passes', () => {
    ...
    const fakeBin = join(fakeBinDir, process.platform === 'win32' ? 'gbrain.cmd' : 'gbrain');
    const fakeReport = '{"ok":true,"skillsDir":"/fake","report":{"ok":true,"issues":[],"summary":{"total_skills":0,"reachable":0,"unreachable":0,"overlaps":0,"gaps":0}},"autoFix":null,"deferred":[{"check":5,"name":"trigger_routing_eval","issue":""},{"check":6,"name":"brain_filing","issue":""}],"error":null,"message":null}';
    writeFileSync(
      fakeBin,
